...
sidebar_position: 1
description: >-
  Learn about the overview of recording capabilities with Ospi. Follow our guide
  for an in-depth understanding of recording features.
---

Ospi records the audio and video of multiple users in a meeting, as well as
interactions with Ospi plugins, in a single file using composite recording mode.

## How does Ospi recording work?

Ospi recordings are powered by anonymous virtual bot users who join your
meeting, record it, and then upload it to Ospi's AWS S3 bucket. For video files,
...
1. When the recording is finished, it is stored in Ospi's AWS S3 bucket.
2. Ospi generates a downloadable link from which the recording can be
   ...
   - Using the
     ...
     webhook. Ospi uses webhooks to notify your application when an event
     happens.
   - Using the
...
3. Download the recording from the download url and store it to your cloud
   storage. The file is kept on Ospi's server for seven days before being
   deleted.
...
<head>
  ...
  <meta name="description" content="Learn about the overview of recording capabilities with Ospi. Follow our guide for an in-depth understanding of recording features."/>
</head>
