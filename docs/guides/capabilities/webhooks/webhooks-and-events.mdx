...
sidebar_position: 2
description: >-
  Learn about the relationship between webhooks and events in Ospi. Follow our
...
3. Handle requests from Ospi by parsing each event object and returning 2xx
   response status codes.
4. Deploy your webhook endpoint so it's a publicly accessible HTTPS URL.
5. Register your publicly accessible HTTPS URL using the Ospi
...
### Step 3: Handle requests from Ospi

Your endpoint must be configured to receive events for the type of event
notifications you want to receive. Ospi sends events to your webhook endpoint as
...
Ospi webhooks have built-in retry methods for `3xx`, `4xx` or `5xx` response
status codes. If Ospi doesn't receive a `2xx` response status code for an event within 3 seconds,
we mark the event as failed and retry up to 5 times.

### Step 4: Secure your webhooks (recommended)

Use [webhook signatures](/guides/capabilities/webhooks/signatures) to verify that Ospi
generated a webhook request and that it didn't come from a malicious server
pretending to be Ospi.
...
[Ospi developer portal](https://dev.dyte.io) or
...
<head>
  ...
  <meta name="description" content="Learn about the relationship between webhooks and events in Ospi. Follow our guide for effective integration and management."/>
</head>
