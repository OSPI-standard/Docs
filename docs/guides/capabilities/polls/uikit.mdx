# UIKit components for Polls

Using Ospi's UIKit you can simplify the process of adding polls in your meetings!

### Prerequisites

1. Install Core SDK for your framework
2. Install UIKit for your framework

## Ospi Poll Form : Create a new poll

If a user has the right set of permissions, the first thing they woulds want to do is create a poll for the meeting. The easiest way to get started is to grab the `Ospi Poll Form` which allows you to create a poll. Check out the implementation based on the framework you are using:

<Tabs groupId="framework">
<TabItem value="web-components" label="Web Components">

```jsx
  <dyte-poll-form id="dyte-el"></dyte-poll-form>
    <script>
      document.getElementById('dyte-el').addEventListener('dyteCreatePoll', (e) => {
      console.log('create poll', e.detail)
      });
    </script>
```

</TabItem>
<TabItem value="react" label="React">

```jsx
<DytePollForm onDyteCreatePoll={(e) => console.log('create poll', e.detail)} />
```

You can control what happens after a poll is creating by using `onDyteCreatePoll` prop.

</TabItem>
<TabItem value="angular" label="Angular">

```jsx
    <dyte-poll-form id="dyte-el"></dyte-poll-form>

    <script>
      document.getElementById('dyte-el').addEventListener('dyteCreatePoll', (e) => {
        console.log('create poll', e.detail);
      });
    </script>
```

</TabItem>
<TabItem value="flutter" label="Flutter">

:::info

This component is not yet available on Flutter.

:::

</TabItem>
<TabItem value="react-native" label="React Native">

```jsx
<DytePollForm onDyteCreatePoll={(e) => console.log('create poll', e.detail)} />
```

</TabItem>
<TabItem value="android" label="Android(Kotlin)">

:::info

This component is not yet available for android development.

:::

</TabItem>
<TabItem value="ios" label="iOS(Swift)">

:::info

This component is not yet available for iOS development.

:::

</TabItem>
</Tabs>

## Ospi Polls : View existing polls

After a user is done creating a poll, they can see all the polls available in a meeting using `Ospi Polls` and enable/disable the polls.

<Tabs groupId="framework">
<TabItem value="web-components" label="Web Components">

```jsx
      <dyte-polls id="dyte-el"></dyte-polls>

      <script>
        document.getElementById('dyte-el').meeting = meeting;
      </script>

      <style>
        dyte-polls {
          height: 480px;
          width: 100%;
          max-width: 320px;
          background-color: '#000';
        }
      </style>
```

</TabItem>
<TabItem value="react" label="React">

```jsx
<DytePolls
  meeting={meeting}
  style={{ height: '480px', maxWidth: '320px', backgroundColor: '#000' }}
/>
```

</TabItem>
<TabItem value="angular" label="Angular">

```jsx
    <dyte-polls id="dyte-el"></dyte-polls>
    <script>
      document.getElementById('dyte-el').meeting = meeting;
    </script>

    <style>
      dyte-polls {
        height: 480px;
        width: 100%;
        max-width: 320px;
        background-color: '#000';
      }
    </style>
```

</TabItem>
<TabItem value="flutter" label="Flutter">

:::info

This component is not yet available on Flutter.

:::

</TabItem>
<TabItem value="react-native" label="React Native">

```jsx
<DytePolls
  meeting={meeting}
  style={{ height: '480px', maxWidth: '320px', backgroundColor: '#000' }}
/>
```

</TabItem>
<TabItem value="android" label="Android(Kotlin)">

:::info

This component is not yet available for android development.

:::

</TabItem>
<TabItem value="ios" label="iOS(Swift)">

:::info

This component is not yet available for iOS development.

:::

</TabItem>
</Tabs>

## Ospi Poll: View poll

The `Ospi Poll` component lets the users view a given poll. It requires a poll object to render the poll.

<Tabs groupId="framework">
<TabItem value="web-components" label="Web Components">

```jsx
      <dyte-poll id="dyte-el"></dyte-poll>

      <script>
        const el = document.getElementById('dyte-el');

        el.addEventListener('dyteVotePoll', (e) => {
          console.log('Voted', e.detail);
        });

        el.poll = {
          id: 'poll-id',
          question: 'Have you started using dyte yet?',
          options: [
            {
              text: 'Yes',
              votes: [{ id: 'vaibhavs-user-id', name: 'Vaibhav' }],
              count: 0,
            },
            {
              text: 'Nope',
              votes: [],
              count: 0,
            },
          ],
          anonymous: false,
          hideVotes: false,
          createdBy: 'Vaibhav',
          createdByUserId: 'vaibhavs-user-id',
          voted: [],
        };
      </script>
```

</TabItem>
<TabItem value="react" label="React">

```jsx
<DytePoll
  poll={{
    id: 'poll-id',
    question: 'Have you started using dyte yet?',
    options: [
      {
        text: 'Yes',
        votes: [{ id: 'vaibhavs-user-id', name: 'Vaibhav' }],
        count: 0,
      },
      {
        text: 'Nope',
        votes: [],
        count: 0,
      },
    ],
    anonymous: false,
    hideVotes: false,
    createdBy: 'Vaibhav',
    createdByUserId: 'vaibhavs-user-id',
    voted: [],
  }}
  onDyteVotePoll={(e) => {
    console.log('Voted', e.detail);
  }}
/>
```

</TabItem>
<TabItem value="angular" label="Angular">

```jsx
      <dyte-poll id="dyte-el"></dyte-poll>

      <script>
        const el = document.getElementById('dyte-el');

        el.addEventListener('dyteVotePoll', (e) => {
          console.log('Voted', e.detail);
        });

        el.poll = {
          id: 'poll-id',
          question: 'Have you started using dyte yet?',
          options: [
            {
              text: 'Yes',
              votes: [{ id: 'vaibhavs-user-id', name: 'Vaibhav' }],
              count: 0,
            },
            {
              text: 'Nope',
              votes: [],
              count: 0,
            },
          ],
          anonymous: false,
          hideVotes: false,
          createdBy: 'Vaibhav',
          createdByUserId: 'vaibhavs-user-id',
          voted: [],
        };
      </script>
```

</TabItem>
<TabItem value="flutter" label="Flutter">

:::info

This component is not yet available on Flutter.

:::

</TabItem>
<TabItem value="react-native" label="React Native">

```jsx
<DytePoll
  poll={{
    id: 'poll-id',
    question: 'Have you started using dyte yet?',
    options: [
      {
        text: 'Yes',
        votes: [{ id: 'vaibhavs-user-id', name: 'Vaibhav' }],
        count: 0,
      },
      {
        text: 'Nope',
        votes: [],
        count: 0,
      },
    ],
    anonymous: false,
    hideVotes: false,
    createdBy: 'Vaibhav',
    createdByUserId: 'vaibhavs-user-id',
    voted: [],
  }}
  onDyteVotePoll={(e) => {
    console.log('Voted', e.detail);
  }}
/>
```

</TabItem>
<TabItem value="android" label="Android(Kotlin)">

:::info

This component is not yet available for android development.

:::

</TabItem>
<TabItem value="ios" label="iOS(Swift)">

:::info

This component is not yet available for iOS development.

:::

</TabItem>
</Tabs>

## Ospi Polls Toggle

Using the `Ospi Polls Toggle`, one can easily change the visibility of a poll on the screen. It requires the user's meeting object to see the unread polls count badge on the component.

<Tabs groupId="framework">
<TabItem value="web-components" label="Web Components">

```jsx
        <dyte-polls-toggle variant="horizontal" class="dyte-el"></dyte-polls-toggle>
        <script>
          const elements = document.getElementsByClassName('dyte-el');
          for (const el of elements) {
            el.meeting = meeting;
          }
        </script>
```

</TabItem>
<TabItem value="react" label="React">

```jsx
<DytePollsToggle variant="horizontal" size="sm" meeting={meeting} />
```

</TabItem>
<TabItem value="angular" label="Angular">

```jsx
      <dyte-polls-toggle variant="horizontal" class="dyte-el"></dyte-polls-toggle>

      <script>
        const elements = document.getElementsByClassName('dyte-el');
        for (const el of elements) {
          el.meeting = meeting;
        }
      </script>
```

</TabItem>
<TabItem value="flutter" label="Flutter">

:::info

This component is not yet available on Flutter.

:::

</TabItem>
<TabItem value="react-native" label="React Native">

```jsx
<DytePollsToggle variant="horizontal" size="sm" meeting={meeting} />
```

</TabItem>
<TabItem value="android" label="Android(Kotlin)">

:::info

This component is not yet available for android development.

:::

</TabItem>
<TabItem value="ios" label="iOS(Swift)">

:::info

This component is not yet available for iOS development.

:::

</TabItem>
</Tabs>

<head>
  <title>UIKit components for Polls Guide</title>
</head>
