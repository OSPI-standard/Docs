...
A toggle button widget designed for controlling the audio state in the Ospi video conference environment. This widget integrates with the Ospi UI Kit and allows users to enable or disable their microphone audio during a meeting.
...
The widget uses `DyteDesignTokens` for theming, which allows for customization according to the Ospi Design System. The `designToken` can be passed as a parameter or will default to the global design token.
